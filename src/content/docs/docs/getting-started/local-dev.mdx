...
3. **Model id.** The site's default `models.ts` uses the NIM-hosted catalog: `llama-3.1-70b-instruct`, `nemotron-70b-instruct`, `mixtral-8x22b-instruct`, `deepseek-r1`. If the API's registry uses different ids, the chat request returns `404 model not registered`.
