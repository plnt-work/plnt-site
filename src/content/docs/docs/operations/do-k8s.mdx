...
In [plnt-site](https://github.com/plnt-work/plnt-site), set `PUBLIC_PLNT_ENDPOINT=https://playground.plnt.work` as a Vercel env var and redeploy. The playground island falls back to a stub if the env var isn't set, so the site is usable even when the API is down.
