...
Already committed at [`plnt/fly.toml`](https://github.com/plnt-work/plnt/blob/main/fly.toml). Points at `docker/playground-api.Dockerfile` and pre-configures the workflow registry. Real backends slot in via the `PLNT_PLAYGROUND_MODELS` env — any OpenAI-compatible upstream (vLLM, TGI, SGLang, or a hosted provider) works.
